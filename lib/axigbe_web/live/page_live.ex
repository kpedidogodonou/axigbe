defmodule AxigbeWeb.PageLive do
  ...
  def handle_event("predict", params, socket)  do
    case params["text"] do
      ...
      #  response = requests.post("http://localhost:8000/poem/invoke", json={'input':{'topic': "my best friend"}})
      ...
      text ->
        ...
        IO.inspect(response)
        req = Poison.decode!(response)
        IO.inspect(req)
        req
        end
            # Nx.Serving.batched_run(MyServing, text) end
           )
  ...
  def handle_info({ref, result}, socket) when socket.assigns.task.ref == ref do
    ...
    {:noreply, assign(socket, task: nil, task: nil, result: label)}
  end
  ...
  def render(assigns) do
    ...
    <div class="h-screen m-auto flex item-center jutify-center antialiased">
      <div class="flex flex-col h-1/2 w-1/2">
        <form class="m-0 flex space-x-2" phx-change="predict">
          <input
            class="block w-full p-2.5 bg-gray-50 border boerder-gray-300 text-gray-900 text-sm rounded-lg"
            type="text"
            name="text"
            phx-debounce="300"
            value={@text}
